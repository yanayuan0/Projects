...
class SigmoidNode(Node):
    ...
    def compute_activation(self):
        ...
        # net_input = 0
        # for edge in self.in_edges:
        #     source_activation = edge.source.activation
        #     weight = edge.weight

        #     # Debug statements
        #     print(f"Source Activation: {source_activation}, Type: {type(source_activation)}")
        #     print(f"Weight: {weight}, Type: {type(weight)}")

        #     if isinstance(source_activation, (int, float)) and isinstance(weight, (int, float)):
        #         net_input += source_activation * weight
        #     else:
        #         raise TypeError("Either source activation or weight is not a numeric type.")
...
class Network(object):
    ...
    def saveWeights(self, filename="bestEver.wts"):
        """
        Saves the current weights to the filename.
        """
        fp = open(filename, "w")
        for layer in self.layers:
            for node in layer:
    ...
    def readWeights(self, filename="bestEver.wts"):
        """
        Retrieves the weights from the filename.
        """
        fp = open(filename, "r")
        weights = []
        for line in fp:
...
if __name__ == "__main__":
    ...
    dataset = ([0,0,0,0,0,2,0,0,2,0,0,0,2,4,0,2], [1.0, 0.0, 0.0, 1.0]), 
    nn = Network(16, 4, [10 for _ in range(10)])
    # Check the initial random weights
    print("initial weights:", nn.getWeights())
    # Overwrite the initial weights with ones that solve OR
    weights = np.random.rand(1204).tolist()  # Create a list of 68 random weights
    ...
    # # Demonstrate that we can successfully get and set weights
    # dataset = ([np.random.rand() for _ in range(16)], [1.0, 0.0, 0.0, 1.0]),
    # ([np.random.rand() for _ in range(16)], [0.0, 1.0, 1.0, 0.0]),
    # nn = Network(16, 4, [])
    # # Check the initial random weights
    # print("initial weights:", nn.getWeights())
    # # Overwrite the initial weights with ones that solve OR
    # weights = np.random.rand(16 * 4 + 4).tolist()  # Create a list of 68 random weights
    # nn.setWeights(weights)
    # print("after setting:", nn.getWeights())
    # # Training should end quickly
    # nn.train(dataset, 1000, verbose=True, random_seed=0)
